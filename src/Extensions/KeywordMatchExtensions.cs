...
public static class KeywordMatchExtensions
{
    ...
    public static List<KeywordConfig> MatchUserName(
        string displayName,
        IEnumerable<KeywordConfig> allKeywords)
    {
        if (string.IsNullOrWhiteSpace(displayName) || allKeywords == null) return new();
        return allKeywords
            .Where(k => k.KeywordType == KeywordType.UserName)
            .Where(k => IsUserNameMatch(displayName, k))
            .ToList();
    }
        ...
        IEnumerable<KeywordConfig> allKeywords)
    {
        ...
        return allKeywords
            .Where(k => k.KeywordType != KeywordType.User && k.KeywordType != KeywordType.UserName)
    ...
    private static bool IsUserNameMatch(string displayName, KeywordConfig cfg)
    {
        if (string.IsNullOrWhiteSpace(cfg.KeywordContent) || string.IsNullOrWhiteSpace(displayName))
            return false;

        var comparison = cfg.IsCaseSensitive ? StringComparison.Ordinal : StringComparison.OrdinalIgnoreCase;
        
        // 根据关键词内容匹配用户显示名称
        return displayName.Contains(cfg.KeywordContent, comparison);
    }
}